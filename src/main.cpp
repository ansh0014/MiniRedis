    ...
    if (!opt.empty()) {
        ...
        if (OPT == "EX") {
            ...
            if (!parseInt(optVal, sec) || sec <= 0) {
                if (delta > 0) tenantMgr.deallocateMemory(tenantId, (size_t)delta);
...
string processCommand(const string &tenantId, const string &raw) {
    ...
    if (cmd == "SET") {
        string key, value;
        if (!(iss >> key)) return "-ERR wrong number of arguments for 'SET'\r\n";
        ...
        if (iss >> opt) {
            ...
            transform(OPT.begin(), OPT.end(), OPT.begin(), ::toupper);
            if (OPT == "EX" || OPT == "PX") {
                if (!(iss >> optVal)) return "-ERR invalid syntax\r\n";
                opt = OPT;
            } else {
                opt.clear();
            }
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (a == "--tenant" && i + 1 < argc) TENANT_ID = argv[++i];
    }
